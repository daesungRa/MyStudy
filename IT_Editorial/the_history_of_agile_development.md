...
agile 은 소프트웨어 개발이 모험적인 것이 될 수 있었던 지점을 볼 수 있는 꼭 필요한 첫걸음이었으며, 그러한 agile 너머에는 삶이 존재한다.
우리가 필요를 느끼게 되는 만큼 빠르게 소프트웨어의 변화를 요구하는 것은 바보같은 것이 될 수 있으나,
그러한 것들이 빠르게 진행될 수 있도록 노력하는 것 자체는 바보같은 것이 아니다.
agile 개념이 우리의 소프트웨어의 지속성과 효과적인 변화를 증진할 수 있는가?
우리는 더 이상 계획될 수 없고 숨을 쉬는 것처럼 일단위, 시단위, 분단위로 발생하는 이러한 이벤트가 일상화되는 가운데
모든 개선점과 함께 소프트웨어를 "배포(release)"할 지점을 알아차릴 수 있는가?

2011년 개최된 "At the Better Software/Agile Development West conference" 에서
Kent Beck 은 어떻게 소프트웨어 공급을 가속화할 수 있는지에 대한 키노트를 발표했다.
그는 agile 에 대한 단 한번의 언급도 없이 한 시간동안 연설했다.

그러나 그가 설명하는 것이 무엇인가에 대한 질문은 없었다.
그는 더 빠른 제품으로써의 소프트웨어를 배포하는 것을 지향하는 트렌트의 관점에서 진술했고,
새로운 버전을 분기별, 월별, 주별, 일별 심지어는 지속적으로 배포할 수 있다는 것의 의미에 대해 논의했다.
"기능의 특징이 정확한 것인가에 대해 알지 못할 때 우리는 그것의 두 가지 버전을 공급하고 사용자들이 결정하도록 해버린다."
이것은 우리가 오늘날 말하는 [지속적 공급](http://techbeacon.com/agile-devops-continuous-delivery-evolution-software-delivery)이나 DevOps 의 전염병이다.

Agile 공정은 그러한 방향성의 첫걸음으로 필요했지만, 지속적 공급의 개념은 더 많은 급진적 변화를 요구한다.
이는 사용자 반응을 기반으로 기존의 것을 즉시 제거하거나 변화시키는 것에 충분히 준비되지 않았음에도
개발자가 그들이 가진 당대 최고의 지식을 기반으로 무엇인가를 시도한다는 것을 의미한다.

그리고 사용자 반응은 용어의 형태보다는 행동의 형태로 나타난다.
우리는 상용 어플리케이션을 모니터링하고 사용자 행위 데이터를 수집한다.
그러한 데이터의 실시간 분석은 팀으로 하여금 그 다음으로 무엇을 할 지,
그리고 더 이상 하루나 이틀을 넘기지 않아서 그 다음으로 무슨 일이 일어날 지 알려준다.
